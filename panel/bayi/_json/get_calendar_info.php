<?php
  ...
  if ((isset($_GET['cities'])) && (isset($_GET['towns'])) && (isset($_GET['streets']))) {
    ...
    if (isset($_SESSION['employee_id'])) {
        $get=mysqli_query($con, "SELECT `customers_service`.`customer_id` as `id`, `customers_service`.`nextservice` as `nextservice`, `customers`.`firstname` as `firstname`, `customers`.`lastname` as `lastname`, `customers_service`.`resolved` as `resolved` FROM `customers_service` INNER JOIN `customers` ON `customers`.`id` = `customers_service`.`customer_id` INNER JOIN `customers_employee` ON `customers`.`id` = `customers_employee`.`customer_id` WHERE (`customers`.`dealer_id` = '$_SESSION[dealer_id]' AND `customers_service`.`status` = '1') AND (`customers_service`.`nextservice` <= '$next_two_month' AND `customers_service`.`nextservice` >= '$prev_two_month') AND (`customers`.`status` = '1' AND `customers`.`deleted_at` = '0') AND (`customers`.`city` = '$city' AND `customers`.`town` = '$town') AND (FIND_IN_SET(`customers`.`street`, $street) AND `customers_employee`.`employee_id` = '$_SESSION[employee_id]')");
    } else {
        $get=mysqli_query($con, "SELECT `customers_service`.`customer_id` as `id`, `customers_service`.`nextservice` as `nextservice`, `customers`.`firstname` as `firstname`, `customers`.`lastname` as `lastname`, `customers_service`.`resolved` as `resolved` FROM `customers_service` INNER JOIN `customers` ON `customers`.`id` = `customers_service`.`customer_id` WHERE (`customers`.`dealer_id` = '$_SESSION[dealer_id]' AND `customers_service`.`status` = '1') AND (`customers_service`.`nextservice` <= '$next_two_month' AND `customers_service`.`nextservice` >= '$prev_two_month') AND (`customers`.`status` = '1' AND `customers`.`deleted_at` = '0') AND (`customers`.`city` = '$city' AND `customers`.`town` = '$town') AND FIND_IN_SET(`customers`.`street`, $street)");
    }
  } elseif ((isset($_GET['cities'])) && (isset($_GET['towns']))) {
    $city=varsql($_GET['cities']);
    $town=varsql($_GET['towns']);
    if (isset($_SESSION['employee_id'])) {
        $get=mysqli_query($con, "SELECT `customers_service`.`customer_id` as `id`, `customers_service`.`nextservice` as `nextservice`, `customers`.`firstname` as `firstname`, `customers`.`lastname` as `lastname`, `customers_service`.`resolved` as `resolved` FROM `customers_service` INNER JOIN `customers` ON `customers`.`id` = `customers_service`.`customer_id` INNER JOIN `customers_employee` ON `customers`.`id` = `customers_employee`.`customer_id` WHERE (`customers`.`dealer_id` = '$_SESSION[dealer_id]' AND `customers_service`.`status` = '1') AND (`customers_service`.`nextservice` <= '$next_two_month' AND `customers_service`.`nextservice` >= '$prev_two_month') AND (`customers`.`status` = '1' AND `customers`.`deleted_at` = '0') AND (`customers`.`city` = '$city' AND `customers`.`town` = '$town') AND `customers_employee`.`employee_id` = '$_SESSION[employee_id]'");
    } else {
    ...
    $city=varsql($_GET['cities']);
    if (isset($_SESSION['employee_id'])) {
        $get=mysqli_query($con, "SELECT `customers_service`.`customer_id` as `id`, `customers_service`.`nextservice` as `nextservice`, `customers`.`firstname` as `firstname`, `customers`.`lastname` as `lastname`, `customers_service`.`resolved` as `resolved` FROM `customers_service` INNER JOIN `customers` ON `customers`.`id` = `customers_service`.`customer_id` INNER JOIN `customers_employee` ON `customers`.`id` = `customers_employee`.`customer_id` WHERE (`customers`.`dealer_id` = '$_SESSION[dealer_id]' AND `customers_service`.`status` = '1') AND (`customers_service`.`nextservice` <= '$next_two_month' AND `customers_service`.`nextservice` >= '$prev_two_month') AND (`customers`.`status` = '1' AND `customers`.`deleted_at` = '0') AND (`customers`.`city` = '$city' AND `customers_employee`.`employee_id` = '$_SESSION[employee_id]')");
    } else {
    ...
    if (isset($_SESSION['employee_id'])) {
        $get=mysqli_query($con, "SELECT `customers_service`.`customer_id` as `id`, `customers_service`.`nextservice` as `nextservice`, `customers`.`firstname` as `firstname`, `customers`.`lastname` as `lastname`, `customers_service`.`resolved` as `resolved` FROM `customers_service` INNER JOIN `customers` ON `customers`.`id` = `customers_service`.`customer_id` INNER JOIN `customers_employee` ON `customers`.`id` = `customers_employee`.`customer_id` WHERE (`customers`.`dealer_id` = '$_SESSION[dealer_id]' AND `customers_service`.`status` = '1') AND (`customers_service`.`nextservice` <= '$next_two_month' AND `customers_service`.`nextservice` >= '$prev_two_month') AND (`customers`.`status` = '1' AND `customers`.`deleted_at` = '0') AND `customers_employee`.`employee_id` = '$_SESSION[employee_id]'");
    } else {
