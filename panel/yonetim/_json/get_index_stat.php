<?php
  ...
  $get_daily=@mysqli_fetch_assoc(mysqli_query($con, "SELECT `first_brand`,`second_brand`,`third_brand`,`total_first_brand`,`total_second_brand`,`total_third_brand` FROM `stats_daily` ORDER BY `id` DESC LIMIT 1"));
  $get_monthly=mysqli_query($con, "SELECT `date`,`completed_services`,`uncompleted_services`,`completed_defects`,`uncompleted_defects` FROM `stats_monthly` ORDER BY `id` DESC LIMIT 7");
  while ($row = @mysqli_fetch_assoc($get_monthly)) {
    $result_monthly[] = $row;
  }
?>
!function (map) {
    ...
    var months = ['Ocak', 'Şubat', 'Mart', 'Nisan', 'Mayıs', 'Haziran', 'Temmuz', 'Ağustos', 'Eylül', 'Ekim', 'Kasım', 'Aralık'];
     ...
     Morris.Line({
        ...
        data: [
          <?php foreach ($result_monthly as $res) { ?>
            { m: '<?=date("Y-m", $res['date'])?>', a: <?=$res['completed_services']?>, b: <?=$res['uncompleted_services']?> },
          <?php } ?>
        ],
        xkey: 'm',
        ykeys: ['a', 'b'],
    ...
    Morris.Line({
       ...
       data: [
         <?php foreach ($result_monthly as $res) { ?>
           { m: '<?=date("Y-m", $res['date'])?>', a: <?=$res['completed_defects']?>, b: <?=$res['uncompleted_defects']?> },
         <?php } ?>
       ],
       xkey: 'm',
       ykeys: ['a', 'b'],
       labels: ['Çözülmüş Arızalar', 'Çözülmemiş Arızalar'],
       hideHover: 'auto',
       resize: true,
       xLabelFormat : function (x) {
         return months[x.getMonth()];
    ...
    var chart = c3.generate({
        ...
        data: {
            columns: [
                ['<?=$get_daily['first_brand']?>', <?=$get_daily['total_first_brand']?>],
                ['<?=$get_daily['second_brand']?>', <?=$get_daily['total_second_brand']?>],
                ['<?=$get_daily['third_brand']?>', <?=$get_daily['total_third_brand']?>]
            ],
            type: 'donut',
            colors: {
                data1: '#ff0000',
