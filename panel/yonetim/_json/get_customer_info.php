<?php
  ...
  $id = varsql($_GET['id']);
  $customer_info = @mysqli_fetch_assoc(mysqli_query($con, "SELECT `dealers`.`currency` as `currency`, `customers`.`firstname` as `firstname`, `customers`.`lastname` as `lastname`, `customers`.`gsm` as `gsm`, `customers`.`address` as `address`, `customers_service`.`nextservice` as `nextservice` FROM `customers` INNER JOIN `customers_service` ON `customers_service`.`customer_id`=`customers`.`id` INNER JOIN `dealers` ON `dealers`.`id`=`customers`.`dealer_id` WHERE `customers`.`id` = '$id' ORDER BY `customers_service`.`id` DESC LIMIT 1"));
  $result_services = array();
  $services=mysqli_query($con, "SELECT `services`.`name` as `name`, `customers_service`.`nextservice` as `nextservice`, `customers_service`.`price` as `price`, `customers_service`.`resolved` as `resolved` FROM `customers_service` INNER JOIN `services` ON `services`.`id`=`customers_service`.`service_id` WHERE `customers_service`.`customer_id` = '$id' AND `customers_service`.`deleted_at` = '0' ORDER BY `customers_service`.`id` ASC LIMIT 5");
...
?>
  ...
  <p class="hidden"></p>
  <p>
    <b>Müşteri Profili</b>: <a onclick="goto('<?=ADMIN_URL?>inspect/customer','none','<?=$id?>','0','0')" href="javascript:void(0)"><span><?=$customer_info['firstname']?> <?=$customer_info['lastname']?></span></a>
  </p>
  <p>
    <b>Sonraki Servis Tarihi</b>: <span><?=date("d-m-Y", $customer_info['nextservice'])?></span>
  </p>
  <p>
    <b>Müşteri Telefon Numarası</b>: <span><?=$customer_info['gsm']?></span>
  </p>
  <p>
    <b>Müşteri Adresi</b>: <span><?=$customer_info['address']?></span>
  </p>
  <p class="text-center lead">Son 5 Servis İşlemi</p>
  <ul class="list-group calendar-services">
    <?php if (!empty($result_services)) { $i=1; foreach ($result_services as $res) { ?>
      <li class="list-group-item<?php if ($res['resolved']==1) { ?> text-success<?php } else { ?> text-warning<?php } ?>">
        ...
        <span class="text-right"><b>Servis Tarihi:&nbsp;<?=date("d-m-Y", $res['nextservice'])?><b></b></b></span><b>
        </b></b>
      </li>
    <?php $i++; } } else { ?>
  ...
  <p class="text-center lead">En Yakın 5 Taksiti</p>
  <ul class="list-group calendar-services">
    <?php if (!empty($result_installments)) { foreach ($result_installments as $res) { ?>
      <li class="list-group-item">
        <span class="text-left"><?=$res['count']?>.&nbsp;</span>
        <span class="text-left"><b>Taksit</b></span>
        &nbsp;-&nbsp;
        <span class="text-left"><b><?=$res['price']?>94<b></b>&nbsp;<?=$customer_info['currency']?></b></span><b>
        <span class="text-right"><b>Taksit Tarihi:&nbsp;<?=date("d-m-Y", $res['date'])?><b></b></b></span><b>
        </b></b>
      </li>
    <?php } } else { ?>
      <li class="list-group-item">
